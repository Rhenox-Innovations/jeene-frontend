...
const SubsubCategoriesListLayer = () => {
  ...
  const [subCategoriesListOld, setsubCategoriesListOld] = useState([]);
  ...
  const onSearchValueChange = (evt) => {
    ...
    if (value && subCategoriesListOld?.length > 0) {
      let filtered = subCategoriesListOld?.filter(
        (x) =>
      ...
      setsubCategoriesList(subCategoriesListOld);
    }
  };

  const getCurrentPageData = () => {
    const startIndex = (currentPage - 1) * pageSize;
    const endIndex = Math.min(startIndex + pageSize, subCategoriesList?.length);
    return subCategoriesList?.slice(startIndex, endIndex);
  };
...
export default SubsubCategoriesListLayer;
